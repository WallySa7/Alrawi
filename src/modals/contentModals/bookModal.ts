...
import { App, TextComponent, DropdownComponent, SliderComponent, Setting, ButtonComponent } from "obsidian";
...
export class BookModal extends BaseModal {
     ...
     */
    constructor(app: App, settings: AlRawiSettings, bookPath: string | null = null) {
        super(app, settings);
    ...
    protected renderModalContent(): void {
        ...
        contentEl.createEl('h2', { text: this.editMode ? 'تعديل كتاب' : 'إضافة كتاب جديد' });
    ...
    private renderBasicInfoSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'المؤلف', () => {
            ...
            this.authorInput.setValue(this.settings.booksSettings.defaultAuthor);
            ...
            this.dataService.getExistingBookAuthors().then(authors => {
    ...
    private renderPublishingInfoSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'سنة النشر', () => {
            ...
            this.publishYearInput.inputEl.max = new Date().getFullYear().toString();
    ...
    private renderReadingInfoSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'حالة القراءة', () => {
            ...
            this.statusInput.setValue(this.settings.booksSettings.defaultStatus);
            return container;
        });

        // Rating slider
        this.createFormField(form, 'التقييم', () => {
            const container = form.createEl('div', { cls: 'alrawi-rating-container' });
    ...
    private renderCategoriesSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'تصنيفات الكتاب', () => {
            ...
            this.categoriesInput.setPlaceholder('تصنيفات مفصولة بفواصل (اختياري)');
            this.categoriesInput.inputEl.addClass('alrawi-input');
            
            // Add category suggestions
            this.dataService.getExistingBookCategories().then(categories => {
                if (categories.length === 0) return;
                
                const categorySuggestions = container.createEl('div', { cls: 'alrawi-tag-suggestions' });
                categories.slice(0, 10).forEach(category => {
                    ...
                    categoryChip.addEventListener('click', () => {
                        const currentCategories = this.categoriesInput.getValue().split(',').map(t => t.trim()).filter(t => t);
                        if (!currentCategories.includes(category)) {
                            const newCategoriesValue = currentCategories.length > 0 
        ...
        this.createFormField(form, 'وسوم', () => {
            ...
            // Add tag suggestions
            this.dataService.getExistingTags('books').then(tags => {
    ...
    private renderCoverSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'رابط صورة الغلاف', () => {
            const container = form.createEl('div', { cls: 'alrawi-url-container' });
        ...
        this.coverPreview = form.createEl('div', { cls: 'alrawi-cover-preview' });
    ...
    private async loadBookData(filePath: string): Promise<void> {
        ...
        try {
            ...
            if (bookData) {
                ...
                this.authorInput.setValue(bookData.author || this.settings.booksSettings.defaultAuthor);
                this.pagesInput.setValue(bookData.pages?.toString() || '');
                this.pagesReadInput.setValue(bookData.pagesRead?.toString() || '0');
                this.isbnInput.setValue(bookData.isbn || '');
                this.publisherInput.setValue(bookData.publisher || '');
                this.publishYearInput.setValue(bookData.publishYear?.toString() || '');
                this.startDateInput.setValue(bookData.startDate || '');
                this.completionDateInput.setValue(bookData.completionDate || '');
                this.languageInput.setValue(bookData.language || 'العربية');
                this.statusInput.setValue(bookData.status || this.settings.booksSettings.defaultStatus);
                this.categoriesInput.setValue(Array.isArray(bookData.categories) ? bookData.categories.join(', ') : (bookData.categories || ''));
                this.tagsInput.setValue(Array.isArray(bookData.tags) ? bookData.tags.join(', ') : (bookData.tags || ''));
    ...
    protected async onSubmit(): Promise<void> {
        ...
        const author = this.authorInput.getValue().trim() || this.settings.booksSettings.defaultAuthor;
        ...
        if (pagesRead > pages) {
            this.showWarning('عدد الصفحات المقروءة لا يمكن أن يتجاوز إجمالي عدد الصفحات');
            return;
        }

        // Set loading state
        this.isLoading = true;
        this.loadingMessage = this.editMode ? 'جاري تحديث بيانات الكتاب...' : 'جاري إضافة الكتاب...';
        this.updateLoadingUI();
        ...
        try {
            ...
            if (this.editMode && this.bookPath) {
                ...
                success = await this.dataService.updateBookNote(this.bookPath, bookData);
                if (success) {
                    this.showSuccess('تم تحديث بيانات الكتاب بنجاح');
                }
            } else {
                // Create new book
                success = await this.dataService.createBookNote(bookData);
                if (success) {
