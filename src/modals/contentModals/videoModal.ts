...
import { App, TextComponent, DropdownComponent, Notice, ButtonComponent } from "obsidian";
import { BaseModal } from "../baseModal";
import { AlRawiSettings } from '../../core/settings';
import { YouTubeService } from "../../services/youtubeService";
import { DataService } from "../../services/dataService";
import { extractVideoId, extractPlaylistId, determineYoutubeUrlType } from "../../utils";
...
export class VideoModal extends BaseModal {
    ...
    private renderUrlSection(form: HTMLElement): void {
        this.createFormField(form, 'رابط اليوتيوب', () => {
            ...
            this.urlInput.setPlaceholder('https://www.youtube.com/watch?v=... أو قائمة');
    ...
    private renderPreviewContainer(form: HTMLElement): void {
        ...
        this.previewContainer = form.createEl('div', { cls: 'alrawi-preview-container' });
        this.previewContainer.style.display = 'none';
        
        this.thumbnailPreview = this.previewContainer.createEl('div', { cls: 'alrawi-thumbnail-container' });
        
        // Video preview container (for embedding iframe)
        this.videoPreviewContainer = this.previewContainer.createEl('div', { cls: 'alrawi-video-embed-container' });
    ...
    private renderContentFields(form: HTMLElement): void {
        ...
        this.createFormField(form, 'اسم الملقي', () => {
            ...
            this.presenterInput.inputEl.setAttr('list', 'presenter-suggestions');
            
            this.dataService.getExistingPresenters().then(presenters => {
        ...
        this.createFormField(form, 'وسوم', () => {
            ...
            // Add tag suggestions
            this.dataService.getExistingTags('videos').then(tags => {
        ...
        this.createFormField(form, 'حالة المشاهدة', () => {
            ...
            this.statusInput.setValue(this.settings.progressTracking.defaultStatus);
            return container;
        });
    }
    ...
    private renderCategoriesSection(form: HTMLElement): void {
        ...
        this.createFormField(form, 'التصنيفات', () => {
            ...
            this.dataService.getExistingVideoCategories().then(categories => {
    ...
    private async previewVideo(): Promise<void> {
        ...
        try {
            ...
            if (urlType === 'video') {
                ...
                const response = await this.youtubeService.fetchVideoDetails(videoId);
                if (!response.success) {
                ...
                const playButton = this.thumbnailPreview.createEl('div', { cls: 'alrawi-play-button' });
    ...
    private async previewPlaylist(playlistId: string): Promise<void> {
        try {
            const response = await this.youtubeService.fetchPlaylistDetails(playlistId);
            if (!response.success) {
            ...
            // Show playlist info
            this.thumbnailPreview.empty();
            const playlistInfo = this.thumbnailPreview.createEl('div', { cls: 'alrawi-playlist-info' });
    ...
    private async handleSingleVideo(url: string): Promise<void> {
        ...
        try {
            ...
            // Get form values
            const presenter = this.presenterInput.getValue().trim() || this.settings.defaultPresenter;
            ...
            const status = this.statusInput?.getValue() || this.settings.progressTracking.defaultStatus;
            ...
            const success = await this.dataService.createVideoNote({
                url,
                videoId,
    ...
    private async handlePlaylist(url: string): Promise<void> {
        ...
        try {
            ...
            // Create the note
            const success = await this.dataService.createPlaylistNote({
                url,
                playlistId,
                title: this.titleInput.getValue().trim() || title,
                presenter,
