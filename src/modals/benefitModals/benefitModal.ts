...
export class BenefitModal extends BaseModal {
	...
	private renderCategoryField(form: HTMLElement): void {
		...
		this.createFormField(form, "تصنيف الفائدة", () => {
			...
			this.createFormField(
				...
				"إضافة تصنيف جديد",
				() => {
					this.customCategoryInput = new TextComponent(
						customCategoryContainer
					);
					this.customCategoryInput.setPlaceholder(
	...
	private renderTagsField(form: HTMLElement): void {
		this.createFormField(form, "الوسوم (اختياري)", () => {
			...
			this.dataService.getTags(contentType).then((tags) => {
	...
	protected async onSubmit(): Promise<void> {
		...
		try {
			...
			const benefitData = {
				...
				pages:
					this.noteType === "book" && this.pagesInput
						? this.pagesInput.getValue().trim()
						: "",
				volume:
					this.noteType === "book" && this.volumeInput
						? this.volumeInput.getValue().trim()
						: "",
			};

			// Save the benefit to the note
			const success = await this.dataService.addBenefit(
				this.noteFile.path,
				benefitData
			);
