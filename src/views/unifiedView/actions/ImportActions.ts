...
export class ImportActions {
     ...
     */
    public showImportDialog(contentType: ContentType, onComplete: () => Promise<void>): void {
        ...
        title.textContent = contentType === CONTENT_TYPE.VIDEOS ? 
        ...
        // Create content area
        const content = dialog.createEl('div', { cls: 'alrawi-dialog-content' });
        ...
        const inputContainer = content.createEl('div', { cls: 'alrawi-file-input-container' });
     ...
     * @param onComplete - Callback to run after import completes
     */
    private async importVideosData(file: File, onComplete: () => Promise<void>): Promise<void> {
        const progressId = this.progressReporter.startProgress('استيراد بيانات الفيديوهات');
        
        try {
            this.progressReporter.updateProgress(progressId, 10, 'جاري قراءة الملف...');
            
            // Read the file
            const jsonData = await this.readFileAsText(file);
            
            this.progressReporter.updateProgress(progressId, 30, 'جاري تحليل البيانات...');
            
            // Import the data with progress updates
            const result = await this.dataService.importVideosData(
                jsonData,
                (progress, message) => {
                    this.progressReporter.updateProgress(
                        progressId,
                        30 + Math.floor(progress * 0.6), // Scale to 30-90%
                        message
                    );
                }
            );
            
            this.progressReporter.updateProgress(progressId, 95, 'جاري تحديث واجهة المستخدم...');
     ...
     * @param onComplete - Callback to run after import completes
     */
    private async importBooksData(file: File, onComplete: () => Promise<void>): Promise<void> {
        const progressId = this.progressReporter.startProgress('استيراد بيانات الكتب');
        ...
        try {
            ...
            // Import the data with progress updates
            const result = await this.dataService.importBooksData(
                jsonData,
                (progress, message) => {
                    this.progressReporter.updateProgress(
                        progressId,
